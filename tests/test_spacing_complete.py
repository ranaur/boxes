...
def test_spacing_complete():
    """Test spacing parameter with complete tuple values"""
    
    print("Testing spacing parameter with complete tuple values...")
    
    # Create a mock environ for testing
    class MockEnviron:
        def __init__(self, path_info="/ABox", query_string="x=100&y=100&h=100&spacing=1:0.5&outside=true&render=4"):
            self.data = {
                "PATH_INFO": path_info,
    ...
    # Test: Export with complete spacing tuple values
    print("\nTesting YAML export with complete spacing tuple (1:0.5):")
    try:
        environ = MockEnviron(query_string="x=100&y=100&h=100&spacing=1:0.5&outside=true&render=4")
        ...
        # Verify the fix
        if yaml_dict.get('spacing') == '(1.0, 0.5)':
            print("\n✅ SUCCESS: Spacing is correctly showing complete tuple values (1.0, 0.5)")
        else:
